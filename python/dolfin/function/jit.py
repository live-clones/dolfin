...
def jit_generate(class_data, module_name, signature, parameters):
    ...
    for k in properties:
        ...
        if isinstance(value, (float, int)):
            members += "      double " + k + ";\n"
            set_props += _set_props.format(key_name=k, name=k, value_shape_check="")
            ...
            check_value_shape = _check_value_shape.format(value_shape=value_shape_str)
            set_generic_function += _set_props.format(key_name=k,
                                                      name="generic_function_" + k,
                                                      value_shape_check=check_value_shape)
    ...
    classname = signature
    code_c = _template_code.format(statement=statement,
                                   classname=classname,
                                   members=members,
...
_template_code = """// Based on https://gcc.gnu.org/wiki/Visibility
...
_get_props = """        if (name == "{key_name}")
          return {name};"""

_set_props = """        if (name == "{key_name}") {{{value_shape_check}
          {name} = _value;
          return;
        }}"""

_check_value_shape = """
          if (_value->value_shape() != std::vector<std::size_t>({{ {value_shape} }}))
            throw std::runtime_error("Expected parameter with value shape ({value_shape})");"""
