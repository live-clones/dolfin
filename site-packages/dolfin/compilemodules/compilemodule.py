...
def compile_extension_module(code, module_name="",
    ...
    *Examples of usage*
    ...
    system_headers = ["cmath", "iostream","complex",
                      "stdexcept","numpy/arrayobject.h",
                      "memory",
                      "dolfin/common/types.h",
                      "dolfin/math/basic.h"]
    system_headers += dolfin_system_headers
    system_headers += additional_system_headers

    header_code = "".join(["#include <%s>\n" % h for h in system_headers])
    cpp_code = header_code + "\n" + code

    dolfin.info("Calling DOLFIN JIT")
...
%%module %s
%%inline %%{"""%module_name + cpp_code + "\n%}"

    temp_dir = tempfile.mkdtemp()
    print(temp_dir)
    module_name = "_" + module_name
    temp_cpp = "%s/%s.cpp" % (temp_dir, module_name)
    temp_i = "%s/%s.i" % (temp_dir, module_name)
    temp_so = "%s/%s.so" % (temp_dir, module_name)
    f = open(temp_i,"w")
    f.write(swig_code)
    f.close()

    cmd = "swig -c++ -python -o %s %s" % (temp_cpp, temp_i)
    print(cmd)
    subprocess.call(cmd.split())

    header_location="-I"+sysconfig.get_python_inc()+" -I"+ffc.get_include_path()+" -I"+dolfin.get_include_path()
    cmd = "c++ %s -shared -fpic -std=c++11 %s -o %s" % (header_location, temp_cpp, temp_so)
    print(cmd)
    subprocess.call(cmd.split())
