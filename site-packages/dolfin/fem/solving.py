...
class LocalSolver(cpp.LocalSolver):
    ...
    def __init__(self, a, L=None, solver_type=cpp.LocalSolver.SolverType_LU):
        ...
        if L is None:
            ...
            L = Form(L) if not L.empty() else cpp.Form(1, 0)
            # Initialize C++ base class
            cpp.LocalSolver.__init__(self, a, L, solver_type)


class LocalPatchSolver(cpp.LocalPatchSolver):

    # Reuse C++ doc-string
    __doc__ = cpp.LocalPatchSolver.__doc__

    def __init__(self, a, L=None,
                 solver_type=cpp.LocalPatchSolver.SolverType_LU,
                 bc_type=cpp.LocalPatchSolver.BCType_none,
                 constraint_type=cpp.LocalPatchSolver.ConstraintType_none):
        """Create a local (patch-wise) solver for a linear variational problem
        a(u, v) = L(v).
        """
        ...
        # Wrap as DOLFIN forms
        a = [Form(ai) for ai in a]
        if L is None:
            # Initialize C++ base class
            cpp.LocalPatchSolver.__init__(self, a,
                                          solver_type, bc_type, constraint_type)
        else:
            L = [Form(Li) if not Li.empty() else cpp.form(1, 0) for Li in L]
            # Initialize C++ base class
            cpp.LocalPatchSolver.__init__(self, a, L,
                                          solver_type, bc_type, constraint_type)
