...
def assemble_multimesh(form,
    ...
    # Create tensor
    tensor = _create_tensor(form, rank, backend, tensor)
...
def _create_tensor(form, rank, backend, tensor):
    ...
    if rank == 0:
        ...
        if backend:
            tensor = backend.create_vector()
        else:
            tensor = cpp.Vector()
    elif rank == 2:
        if backend:
            tensor = backend.create_matrix()
        else:
            tensor = cpp.Matrix()
    else:
...
class SystemAssembler(cpp.SystemAssembler):
    ...
    def __init__(self, A_form, b_form,
        ...
        * Arguments *
        ...
        # Check for lists of forms, and bcs, for block assembly of 2x2 system
        if hasattr(A_form, "__iter__") and hasattr(b_form, "__iter__") \
           and hasattr(bcs, "__iter__") and len(bcs)==2 and hasattr(bcs[0], "__iter__") \
           and hasattr(bcs[1], "__iter__") :
            A_dolfin_forms = [_create_dolfin_form(A, form_compiler_parameters) for A in A_form]
            b_dolfin_forms = [_create_dolfin_form(b, form_compiler_parameters) for b in b_form]
            cpp.SystemAssembler.__init__(self, A_dolfin_forms, b_dolfin_forms, bcs[0], bcs[1])
        else:
            # Simple case of single forms, and (possible) list of bcs
            A_dolfin_form = _create_dolfin_form(A_form, form_compiler_parameters)
            b_dolfin_form = _create_dolfin_form(b_form, form_compiler_parameters)
            # Check bcs
            bcs = _wrap_in_list(bcs, 'bcs', cpp.DirichletBC)
