  ...
  std::vector<double>* values)
{
  ...
  for (CellIterator cell(mesh); !cell.end(); ++cell)
  {
	...
	offset += dmap.size();
      ...
      // Needed to resize the mixed_A matrix
      mixed_A_size *= dofs[i].size();
    }
    ...
    // NOTE : We might need to resize the local tensor (ufc.mixed_A)
    // in case we consider a submesh of codim 1 located on the boundary.
    // Indeed, the dimension of the local tensor generated by ffc
    // assumes that each cell of a submesh of dim(n-1) is shared by
    // two cells of the parent mesh of dim(n) (interior facet).
    // If the submesh is a part of the boundary, there is only one cell
    // involved (exterior facet) and we need to remove the zero
    // values corresponding to the hypothetical ajdacent cell.

    // Local copy of ufc.mixed_A
    // (We don't want to modify the ufc object)
    auto mixed_A = ufc.mixed_A;

    // NOTE : This handles only the case codimension 1
    if(form_rank > 1 && !(mixed_A.size() == mixed_A_size))
    {
      for(int r=0; r<form_rank; ++r)
      {
	if(codim[r] != 1)
	  continue;

	std::size_t idx = dofs[1].size();
	if(r == 0)
	  idx *= dofs[0].size();

	for(int i=0; i<dofs[0].size(); ++i)
	{
	  mixed_A.erase(mixed_A.begin() + idx, mixed_A.begin() + idx + dofs[1].size());
	  if(r == 1)
	    idx += dofs[1].size();
	}
      }
    ...
    if (is_cell_functional)
      (*values)[cell->index()] = mixed_A[0];
    else
      A.add_local(mixed_A.data(), dofs);
