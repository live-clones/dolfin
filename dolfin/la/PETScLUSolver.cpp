...
namespace
{
  const MATSOLVERTYPE get_solver_package_type(KSP ksp)
  {
    ...
    ierr = PCFactorGet ## MATSOLVERTYPE(pc, &solver_type);
    if (ierr != 0) dolfin::PETScObject::petsc_error(ierr, __FILE__, "PCFactorGet" #MATSOLVERTYPE);
                             ...
                             std::string method) : _solver(comm)
{
  ...
  ierr = PCFactorSet ## MATSOLVERTYPE(pc, solver_package);
  if (ierr != 0) PETScObject::petsc_error(ierr, __FILE__, "PCFactorSet" #MATSOLVERTYPE);
