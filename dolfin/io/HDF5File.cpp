                    ...
                    bool use_partition_from_file) const
{
  ...
  // Only read on valid comms
  if (!MPI::is_null(_mpi_comm))
  {
    if (MPI::size(input_mesh.mpi_comm()) != MPI::size(_mpi_comm))
      std::cout << "Mesh and HDF5 have different size communicators \n";
