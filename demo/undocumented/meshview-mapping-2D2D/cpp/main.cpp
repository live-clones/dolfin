...
int main()
{
  ...
  auto V = std::make_shared<MeshView_2D2D::FunctionSpace>(mesh);
  ...
  auto V1=std::make_shared<MeshView_2D2D::FunctionSpace>( submesh1 );
  auto V2=std::make_shared<MeshView_2D2D::FunctionSpace>( submesh2 );

  // Bilinear and linear forms
  MeshView_2D2D::BilinearForm a(V, V);
  MeshView_2D2D::LinearForm L(V);

  MeshView_2D2D::BilinearForm a1(V1, V1);
  MeshView_2D2D::BilinearForm a2(V2, V2);
  MeshView_2D2D::LinearForm L1(V1);
  MeshView_2D2D::LinearForm L2(V2);
  ...
  XDMFFile out_global(mesh->mpi_comm(), "meshview-mapping-2D2D-global.xdmf");
  XDMFFile out_sub1(mesh->mpi_comm(), "meshview-mapping-2D2D-subdomain1.xdmf");
  XDMFFile out_sub2(mesh->mpi_comm(), "meshview-mapping-2D2D-subdomain2.xdmf");
  if( has_hdf5() )
    {
      out_global.write(u);
      out_sub1.write(u1);
